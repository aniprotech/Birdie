...
Foreground location, camera, photo-library, microphone and speech-recognition permission descriptions are declared for both store builds. Check-in starts an auditable foreground location trail for the active visit and checkout stops it. Photos can include capture time, coordinates and accuracy. Location is never collected before check-in or after checkout. The app signs a user out after five minutes without interaction, including time spent in the background. Offline visit mutations already saved on the device are retained during an inactivity logout and remain owned by that caregiver; they can resume synchronisation only after that caregiver signs in again. A manual sign-out clears the local pending queue.
