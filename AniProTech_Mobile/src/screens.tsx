...
export function Visits({user}:{user:User}) {
  ...
  async function addPhoto() {
    ...
    const result=await ImagePicker.launchCameraAsync({mediaTypes:["images"],quality:.7,exif:false}); if(result.canceled)return;
    setBusy(true); try{let metadata:{latitude:number|null;longitude:number|null;accuracy:number|null;capturedAt:string}={latitude:null,longitude:null,accuracy:null,capturedAt:new Date().toISOString()};try{const position=await Location.getCurrentPositionAsync({accuracy:Location.Accuracy.High});metadata={latitude:position.coords.latitude,longitude:position.coords.longitude,accuracy:position.coords.accuracy,capturedAt:new Date(position.timestamp).toISOString()}}catch{}const asset=result.assets[0];await upload(`/api/mobile/visits/${selected.id}/photos`,{uri:asset.uri,fileName:asset.fileName,mimeType:asset.mimeType,fileSize:asset.fileSize},caption,metadata);setDetail(await api(`/api/mobile/visits/${selected.id}`));setCaption("");Alert.alert("Photo uploaded","The care evidence is now attached to this visit and visible to authorised staff.");}catch(e){Alert.alert("Photo could not be uploaded",(e as Error).message)}finally{setBusy(false)}
  }
