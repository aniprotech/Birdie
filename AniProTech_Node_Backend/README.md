...
- SMTP is opt-in. `MAIL_MODE=outbox` writes local files. A `SHARE_ACCESS_URL` for your own client portal must be configured before sharing links; the application does not send client information to the unrelated third-party URL embedded in the Java source.
