...
try {
 ...
 await page.goto(url);await page.getByRole('heading',{name:'Upcoming time off'}).waitFor();await page.screenshot({path:'../time-off-checks/desktop.png'});
 ...
 await page.getByRole('button',{name:'Set holiday year'}).click();await page.getByLabel('Start month',{exact:true}).selectOption('4');await page.getByRole('button',{name:'Save settings'}).click();await page.getByRole('button',{name:'Holiday year settings'}).waitFor();
 ...
 console.log('PASS: live Time off read; browser fixtures verify booking, cancellation, reload, holiday settings and mobile layout. No real leave or agency settings changed.');
