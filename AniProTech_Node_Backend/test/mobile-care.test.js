...
test("Mobile carer visit workflow is assigned, audited and tenant scoped", async () => {
  ...
  try {
    ...
    const medicationId=detailResponse.body.results.data.medication[0].id,clientEventId=randomUUID();
    r=await call("post",`/api/mobile/visits/${visitId}/attendance`,{event:"CHECK_OUT",latitude:52.1,longitude:-1.5,accuracy:9});assert.equal(r.status,409);
    ...
    assert.equal(detail.visit.status,"COMPLETED"); assert.equal(detail.attendance.length,2); assert.equal(detail.medicationAdministrations.length,2); assert.equal(detail.medicationAdministrations[0].outcome,"ADMINISTERED"); assert.equal(detail.medicationAdministrations[1].outcome,"REFUSED"); assert.equal(detail.entries.length,4); assert.equal(detail.entries.some((e)=>e.category==="MEDICATION"&&e.kind==="ALERT"),true); assert.equal(detail.attachments.length,1); assert.equal(detail.entries.some((e)=>e.title==="Caregiver arrived"),true); assert.equal(sent.some((m)=>m.subject==="Your caregiver has arrived"),true);
