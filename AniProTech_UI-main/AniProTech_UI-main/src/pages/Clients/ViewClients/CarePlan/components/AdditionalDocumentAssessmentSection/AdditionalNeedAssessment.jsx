...
import { createOrUpdateAdditionalAssessmentAPI, deleteAdditionalAssessmentAPI } from "../additionalAssessmentAPIEndpoint/assessmentAPIEndpoint";
import TextField from "../../../../../../components/TextInput/TextInput";

const AdditionalNeedsAssessment = ({ clientName, assessmentData, onAssessmentUpdate }) => {
    ...
    const [additonalAssessmentName, setAdditonalAssessmentName] = useState("");
    ...
    const handleStartAssessment = async () => {
        try {
            ...
            const initialPayload = {
                ...
                name: additonalAssessmentName ? additonalAssessmentName : null,
            };

            const endpoint = createOrUpdateAdditionalAssessmentAPI(assessmentType, "create", null, clientsPersonalDetailData?.id);
            ...
            if (response?.data?.error === false) {
                ...
                navigate(
                    `/admin/clients/${clientsPersonalDetailData?.id}/care-plan/${assessmentType}/additional-assessment?mode=create&id=${createdAssessmentId}`,
                    {
                        state: { assessmentName: additonalAssessmentName },
                    },
                );
            } else {
    ...
    const handleUpdateAssessment = async (assessmentId) => {
        ...
        try {
            ...
            if (response?.data?.error === false) {
                ...
                navigate(
                    `/admin/clients/${clientsPersonalDetailData?.id}/care-plan/${assessmentType}/additional-assessment?mode=update&id=${assessmentId}&updateId=${updateId}`,
                    {
                        state: { assessmentName: additonalAssessmentName },
                    },
                );
    ...
    const handleReviewAssessment = async () => {
        ...
        try {
            ...
            if (response?.data?.error === false) {
                ...
                navigate(
                    `/admin/clients/${clientsPersonalDetailData?.id}/care-plan/${assessmentType}/additional-assessment?mode=review&id=${assessmentToReview.id}&updateId=${reviewId}`,
                );
            } else {
    ...
    const handleViewAssessmentDetails = (assessment) => {
        ...
        if (status === "In progress") {
            navigate(
                `/admin/clients/${clientsPersonalDetailData?.id}/care-plan/${assessmentType}/additional-assessment?mode=update&id=${assessment.id}`,
            );
        } else if (status === "Review in progress") {
            navigate(
                `/admin/clients/${clientsPersonalDetailData?.id}/care-plan/${assessmentType}/additional-assessment?mode=review&id=${assessment.id}`,
            );
        } else {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            setIsDeleting(true);
            const endpoint = deleteAdditionalAssessmentAPI(assessmentType, assessmentToDelete.id);
    ...
    const restrictReview = assessmentType === "condition-specific" || assessmentType === "control-substances";
    ...
    return (
        <>
            ...
            >
                {!restrictReview && (
                    <>
                        <h2 className="poppins-medium text-lg text-customBlack"> Needs assessment</h2>
                        <p className="mb-3 text-sm text-customFeedCardGreyText1">
                            Record {clientName || "client"}&apos;s level of independence for each everyday activities activity, and any support that
                            is required
                        </p>
                    </>
                )}
                ...
                {restrictReview && (
                    <div className="mb-4">
                        <TextField
                            label="To start, tell us which condition requires a specific assessment."
                            type="text"
                            value={additonalAssessmentName}
                            valueChange={(e) => setAdditonalAssessmentName(e)}
                            componentName="NormalValidation"
                        />
                        <button
                            onClick={handleStartAssessment}
                            title={restrictReview && !additonalAssessmentName ? "Please enter a condition name" : ""}
                            disabled={isCreatingAssessment || (restrictReview && !additonalAssessmentName)}
                            className="flex min-w-[160px] mt-4 items-center justify-center rounded border border-customNavy/50 px-4 py-2 text-sm text-customNavy hover:bg-customTextLightNavy/10 disabled:opacity-50"
                        >
                ...
                {!hasCompletedAssessment && !restrictReview && (
                    <button
                        onClick={handleStartAssessment}
                        title={restrictReview && !additonalAssessmentName ? "Please enter a condition name" : ""}
                        disabled={isCreatingAssessment || (restrictReview && !additonalAssessmentName)}
                        className="flex min-w-[160px] items-center justify-center rounded border border-customNavy/50 px-4 py-2 text-sm text-customNavy hover:bg-customTextLightNavy/10 disabled:opacity-50"
                    >
                        {isCreatingAssessment ? (
                            <InnerLoader
                                loading={true}
                ...
                {hasCompletedAssessment && hasAssessmentForReview && !restrictReview && (
                    <button
                            ...
                            {assessmentData.map((assessment, index) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className="flex cursor-pointer items-center justify-between rounded-md border border-gray-300 px-8 py-4 hover:bg-gray-50"
                                    ...
                                    >
                                        <div>
                                            ...
                                            {assessment?.name && (
                                                <p className="text-sm text-customFeedCardGreyText1">{assessment?.name ? assessment?.name : "-"}</p>
                                            )}
                                        ...
                                        <div className="flex h-full flex-col items-end justify-between">
                                            ...
                                            {showUpdate && (
                                                <button
                                                    ...
                                                    className="poppins-semibold flex items-center justify-center py-2 text-sm text-customTextLightNavy hover:text-customTextLightNavy/80 disabled:opacity-50"
                                                >
...
AdditionalNeedsAssessment.propTypes = {
...
export default AdditionalNeedsAssessment;
