...
import TeamsClientsActionMenu from "./ActionMenu";
...
const ClientCareRecipientTable = ({
    ...
    const toggleRow = (clientId) => {
        setSelectedIds((prev) => (prev.includes(clientId) ? prev.filter((x) => x !== clientId) : [...prev, clientId]));
    };
        ...
        () => [
            {
                ...
                header: ({ table }) => (
                    <div className="px-1">
                        <input
                            ...
                            onChange={() => {
                                const allCarerIds = table.getRowModel().rows.map((row) => row.original.clientId);
                ...
                cell: ({ row }) => (
                    ...
                    >
                        <input
                            type="checkbox"
                            checked={selectedIds.includes(row.original.clientId)}
                            onChange={() => toggleRow(row.original.clientId)}
                            className="h-4 w-4 rounded border-gray-300 text-blue-600"
                        />
                    </div>
                ),
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            const updatedCareTeam = selectedIds
                .map((clientId) => {
                    const carerData = data.find((item) => item.clientId === clientId);
                    if (carerData) {
            ...
            await _put(APIConfig.TEAMS.TEAM_CLIENTS_BULK_UPDATE(clientId), updatedCareTeam);
                ...
                <tbody>
                    {loading ? (
                        ...
                        table.getRowModel().rows.map((row) => {
                            const isSelected = selectedIds.includes(row.original.clientId);
                            return (
                                <tr
                                    key={row.id}
                                    onClick={() => toggleRow(row.original.clientId)}
