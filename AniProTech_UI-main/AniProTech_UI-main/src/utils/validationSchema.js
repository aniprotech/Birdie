...
export const clientContactValidationSchema = Yup.object({
    ...
    additionalPhoneType: Yup.string().when("additionalPhone", {
        is: (val) => val?.trim() !== "",
        then: (schema) => schema.required("Additional phone type is required."),
        otherwise: (schema) => schema.notRequired(),
    }),
});
...
export const generateFullValidationSchema = (initialValues, skipClientContactValidation = false) => {
    return Yup.object().shape({
        ...
        // ...(skipClientContactValidation ? {} : clientContactValidationSchema.fields),
