...
const ClientsCareTeamIndex = () => {
    ...
    const fetchCareTeamData = async (searchValue = searchTerm) => {
        ...
        try {
            const response = await _post(APIConfig.CLIENT_CARE_TEAM.GET_BY_CLIENT(clientId), payload);
            const careTeamData = response?.data?.results?.data?.data?.careTeam || [];
...
export default ClientsCareTeamIndex;
