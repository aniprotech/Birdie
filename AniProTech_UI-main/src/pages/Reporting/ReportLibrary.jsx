...
export default function ReportLibrary({ from, to, graceMinutes }) {
    const [data, setData] = useState(null), [error, setError] = useState(""), [loading, setLoading] = useState(true);
    const [selected, setSelected] = useState("reported"), [search, setSearch] = useState("");
    useEffect(() => {
        let active = true;
    ...
    const report = reports.find(([key]) => key === selected) || reports[0];
    ...
    return <div className="space-y-6">
        <div className="rounded-xl border border-teal-100 bg-teal-50 p-5"><h2 className="text-xl font-semibold text-slate-900">Caremonitor report library</h2><p className="mt-1 text-sm text-slate-600">Care delivery and quality reports calculated from your organisation’s own records. Select a report to view its trend and detail. All figures use the date range above.</p></div>
        {groups.map((group) => <section key={group.title}><h3 className="text-lg font-semibold text-slate-900">{group.title}</h3><p className="mb-3 text-sm text-slate-500">{group.description}</p><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{group.reports.map(([key, title, description]) => <button type="button" key={key} onClick={() => setSelected(key)} className={`rounded-xl border p-4 text-left transition-colors hover:border-teal-500 hover:bg-teal-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-teal-700 ${selected === key ? "border-teal-600 bg-teal-50" : "bg-white"}`}><span className="text-sm font-semibold text-slate-900">{title}</span><span className="mt-2 block text-xs text-slate-500">{description}</span><span className="mt-3 block text-xs font-medium text-teal-800">Open report →</span></button>)}</div></section>)}
        <section className="space-y-5 rounded-xl border bg-white p-5" aria-live="polite"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-teal-700">Live report</p><h3 className="text-xl font-semibold text-slate-900">{report[1]}</h3><p className="mt-1 text-sm text-slate-600">{report[2]}</p></div><button className={buttonClass} disabled={!data || data.truncated} onClick={exportRows}>Export {isAlertReport ? "alerts" : "visit details"}</button></div>
