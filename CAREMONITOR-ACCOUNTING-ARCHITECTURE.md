...
- The future TrueLayer sandbox configuration uses application name `Ani Business Sandbox` and client ID `sandbox-anibusinesssandbox-f5ee11`. These are identifiers, not bank credentials; no TrueLayer consent, account retrieval or transaction sync is active yet.
