...
- Rotate any database, Gmail OAuth and Google Maps credentials that were present in local environment files or chat history.
...
- Set `NODE_ENV=production`, `DB_DRIVER=postgres`, `DATABASE_URL`, a random `JWT_SECRET` of at least 32 characters, `FRONTEND_URL`, `CORS_ORIGINS`, `MAIL_MODE=gmail`, `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN`, and `GMAIL_SENDER`.
...
- Back up PostgreSQL and uploaded files together, test restoration, monitor `/api/health` for liveness, and use `/api/ready` as the deployment readiness check.
...
- Complete physical-device checks for login deep links, camera, photo upload, microphone transcription, foreground location, arrival notification, check-in/out, eMAR recording, encrypted offline retry, reconnection and permission denial.
...
- Deploy staging web and API builds and run a complete carer visit from scheduling through eMAR/tasks/notes/media, alerts, checkout, care-log review and finance confirmation.
